...
class TradingEngine:
    ...
    def generate_recommendation(self, stock_data, news_articles=None):
        ...
        rsi = self.calculate_rsi(stock_data).iloc[-1]
        ...
        try:
            ...
            if action.upper() == "BUY":
                if ticker in self.portfolio:
                    total_qty = self.portfolio[ticker]['quantity'] + quantity
                    avg_price = (
                        self.portfolio[ticker]['avg_price'] * self.portfolio[ticker]['quantity'] + trade_value
                    ) / total_qty
                    self.portfolio[ticker]['quantity'] = total_qty
                    self.portfolio[ticker]['avg_price'] = avg_price
                else:
